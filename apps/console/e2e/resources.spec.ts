...
const listPages = [
  ...
  ["Customers", "customers"],
  ["Invoices", "invoices"],
  ["Service accounts", "developer/service-accounts"],
  ...
  await page.route("**/admin/v1/organizations/*/meters", async (route) => {
    if (route.request().method() === "POST") {
      ...
      return route.fulfill({
        ...
        body: JSON.stringify({ meter: { ID: "meter-1", ...payload } }),
      });
    }
...
const failures = [
  ...
  [422, "VALIDATION_FAILED", "Invalid resource"],
  [500, "INTERNAL", "Unable to save resource"],
] as const;

for (const [status, code, message] of failures) {
  test(`surfaces ${status} ${code} from resource mutation`, async ({
    page,
  }) => {
    await mockAPI(page, {
      failure: { method: "POST", path: /\/customers$/, status, code, message },
    });
