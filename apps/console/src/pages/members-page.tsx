...
export function MembersPage() {
  ...
  async function load() {
    try {
      const [userResult, roleResult, policyResult] = await Promise.all([
        client.users(pagination.request),
      ...
      pagination.setPageInfo(userResult.page_info);
    } catch (cause) {
      setError(
  ...
  const members = useMemo(() => {
    ...
    for (const binding of policy.bindings) {
      if (
        binding.principal.type !== "user" ||
        binding.principal.issuer !== "billing-console"
      )
        continue;
      grouped.set(binding.principal.subject, [
        ...(grouped.get(binding.principal.subject) ?? []),
        binding.role,
      ]);
    }
    return users
      .filter((user) => grouped.has(user.id))
      .map((user) => ({ ...user, roles: grouped.get(user.id) ?? [] }));
  }, [policy, users]);
    ...
    try {
      const duplicate = policy.bindings.some(
        (binding) =>
          binding.principal.type === "user" &&
          binding.principal.issuer === "billing-console" &&
          binding.principal.subject === userID &&
  ...
  async function removeMember(member: Member) {
    ...
    setError("");
    try {
      const updated = await client.setPolicy({
        ...policy,
        bindings: policy.bindings.filter(
          (binding) =>
            !(
              binding.principal.type === "user" &&
              binding.principal.issuer === "billing-console" &&
              binding.principal.subject === member.id
            ),
        ),
      });
      setPolicy(updated);
  ...
  async function updateMemberRoles(member: Member, nextRoles: string[]) {
    ...
    setError("");
    try {
      const otherBindings = policy.bindings.filter(
        (binding) =>
          !(
            binding.principal.type === "user" &&
            binding.principal.issuer === "billing-console" &&
      ...
      const memberBindings = nextRoles.map((role) => ({
        role,
        principal: {
          type: "user" as const,
          issuer: "billing-console",
      ...
      const updated = await client.setPolicy({
        ...policy,
        bindings: [...otherBindings, ...memberBindings],
      });
      setPolicy(updated);
    } catch (cause) {
      setError(
    ...
    <main className="content">
      <div className="page-head">
        <div>
          ...
          <h1>Members</h1>
          <p className="muted">Manage user access to this organization.</p>
        </div>
      </div>
      <Card className="mb-6">
        <CardHeader>
        ...
        <CardContent>
          <FieldGroup className="grid gap-4 md:grid-cols-[1fr_1fr_auto] md:items-end">
            <Field>
              <FieldLabel hint="Console user who will receive organization access.">
                User
              </FieldLabel>
              <RelationCombobox
                value={userID}
            ...
            <Field>
              <FieldLabel hint="IAM role whose permissions will be granted to the selected user.">
                Role
              </FieldLabel>
              <RelationCombobox
                value={roleName}
              ...
              },
              {
                ...
                cell: ({ row }) => (
                  <RelationMultiCombobox
                    values={row.original.roles}
                    onValuesChange={(values) =>
                      void updateMemberRoles(row.original, values)
                    }
                    options={roles.map((role) => ({
                      value: role.name,
                      label: role.display_name,
                ...
                cell: ({ row }) => (
                  <Button
                    ...
                    onClick={(event) => {
                      ...
                      void removeMember(row.original);
                    }}
                  >
                    <Trash2 />
                  </Button>
