...
func (r *Repository) Update(ctx context.Context, v domain.Subscription) (domain.Subscription, error) {
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("organization_id=? AND subscription_id=?", v.OrganizationID, v.ID).Delete(&itemModel{}).Error; err != nil {
			return err
		}
		for _, i := range v.Items {
			if err := tx.Create(toItemModel(i)).Error; err != nil {
...
func (r *Repository) load(ctx context.Context, m subscriptionModel) (domain.Subscription, error) {
	...
	for _, i := range items {
		v.Items = append(v.Items, domain.Item{ID: i.ID, OrganizationID: i.OrganizationID, SubscriptionID: i.SubscriptionID, PriceID: i.PriceID, CreatedAt: i.CreatedAt, UpdatedAt: i.UpdatedAt})
	}
	return v, nil
}
...
func toItemModel(v domain.Item) *itemModel {
	return &itemModel{ID: v.ID, OrganizationID: v.OrganizationID, SubscriptionID: v.SubscriptionID, PriceID: v.PriceID, CreatedAt: v.CreatedAt, UpdatedAt: v.UpdatedAt}
}
