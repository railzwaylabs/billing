...
func TestResourcesUsesFixedQueriesAndSelectedRange(t *testing.T) {
	...
	result, err := NewService(querier, clock.Fixed{Time: now}).Resources(context.Background(), domain.PeriodWeek)
	if err != nil {
		t.Fatal(err)
	}
	if result.Range != domain.PeriodWeek || result.Step != int64((6*time.Hour).Seconds()) {
...
func TestResourcesRejectsUnknownRangeWithoutQueryingPrometheus(t *testing.T) {
	querier := &recordingQuerier{}
	_, err := NewService(querier, clock.Fixed{}).Resources(context.Background(), domain.Period("year"))
